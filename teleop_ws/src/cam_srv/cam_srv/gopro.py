...
class GoPro(Node):
    def __init__(self,max_queue_size=128):
    ...
    async def connect_to_gopro(self,):
        try:
            ...
            if isready:
                self.connected = True
                self.get_logger().info("Yay! I'm connected via USB, opened, and ready to send / get data now!")
            else:
                self.get_logger().error("Boo! I'm not connected via USB, not opened, and not ready to send / get data now!")
        except Exception as e:
            self.get_logger().error(f"Failed to connect to GoPro: {e}")
    
    async def start_webcam(self):
        try:
            await self.gopro.http_command.wired_usb_control(control=constants.Toggle.DISABLE)
            ...
            if (status := (await self.gopro.http_command.webcam_start()).data.error) != constants.WebcamError.SUCCESS:
                self.get_logger().info(f'{await self.gopro.http_command.webcam_start()}')
